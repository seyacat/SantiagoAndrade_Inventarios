...
public class HttpRequestTest {
	...
	public void testStockChange() throws URISyntaxException {
		...
		assertThat(
				postRequest("http://localhost:" + port + "/product/stock/-50", "{\"stock\":10}").get("status").asInt())
				.isEqualTo(400);
		// TEST STOCK LESS OR SAME THAN 0
		assertThat(postRequest("http://localhost:" + port + "/product/stock/2", "{\"stock\":0}").get("status").asInt())
				.isEqualTo(400);
		assertThat(
				postRequest("http://localhost:" + port + "/product/stock/2", "{\"stock\":-14}").get("status").asInt())
				.isEqualTo(400);
	...
	public void testCrudClient() throws URISyntaxException {
		...
		// CREATE CLIENT
		// MISSING CI
		assertThat(postRequest("http://localhost:" + port + "/client/create", "{\"name\":\"Pedro\"}").get("status")
				.asInt()).isEqualTo(400);
		// MISSING NAME
		assertThat(postRequest("http://localhost:" + port + "/client/create", "{\"ci\":\"1700000001\"}").get("status")
				.asInt()).isEqualTo(400);
		// CREATE FOR UPDDATE
		...
		if (idNode != null) {
			// TEST DELETE
			assertThat(
					getRequest("http://localhost:" + port + "/client/delete/" + idNode.asInt()).get("status").asInt())
					.isEqualTo(200);
		}
		// TEST CREATE
		Integer id = postRequest("http://localhost:" + port + "/client/create",
				"{\"name\":\"Pedro\",\"ci\":\"1700000001\"}").get("id").asInt();
		assertThat(id).isNotNull();
		// TEST UPDATE
		assertThat(postRequest("http://localhost:" + port + "/client/update/" + id,
				"{\"name\":\"Pedro\",\"photo\":\"/photo/path/01.jpg\"}").get("status").asInt()).isEqualTo(200);
	}
	...
	public void testOrder() throws URISyntaxException {
		...
		// ASYNC REQUEST TEST
		assertThat(postRequest("http://localhost:" + port + "/order/create",
				"{\"client_id\":1,\"stores\":[{\"store_id\":1,\"products\":[{\"id\":1,\"qty\":13}]}]}", false).get("id")
				.asInt()).isNotNull();
		...
		// SYNC REQUEST TEST
		assertThat(postRequest("http://localhost:" + port + "/order/create",
				"{\"client_id\":1,\"stores\":[{\"store_id\":1,\"products\":[{\"id\":2,\"qty\":15}]}]}", false).get("id")
				.asInt()).isNotNull();
	...
	public void testReports() throws URISyntaxException {
		...
		// TEST CSV
		assertThat(
				getStringRequest("http://localhost:" + port + "/report/csv/1?start=2022-05-01&end=2022-05-02", false))
				.isNotBlank();
		assertThat(
				getStringRequest("http://localhost:" + port + "/report/csv/1?start=2022-04-01&end=2022-04-02", false))
				.isNotBlank();
		assertThat(getRequest("http://localhost:" + port + "/report/csv/1?start=hola&end=mundo", false).get("status")
				.asInt()).isEqualTo(400);
		assertThat(
				getRequest("http://localhost:" + port + "/report/csv/1?start=2022-05-01", false).get("status").asInt())
				.isEqualTo(400);
		assertThat(getRequest("http://localhost:" + port + "/report/csv/1?end=2022-05-01", false).get("status").asInt())
				.isEqualTo(400);
		assertThat(getRequest("http://localhost:" + port + "/report/csv/-1?start=2022-04-01&end=2022-04-02", false)
				.get("status").asInt()).isEqualTo(404);
	}
